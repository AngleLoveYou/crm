...
import com.wy.utils.PaymentUtil;
import com.wy.domain.Cart;
import com.wy.domain.CartItem;
import com.wy.domain.Category;
import com.wy.domain.Order;
import com.wy.domain.OrderItem;
...
public class ProductSerlvet extends BaseServlet {
	...
	public void myOrders(HttpServletRequest request, HttpServletResponse response)
		...
		// 查询该用户所有订单信息（单表查询orders）
		// 集合中的每一个Order对象的数据是不完整的缺少orderItems数据
		...
		if (orderList != null) {
			for (Order order : orderList) {
				...
				// 查询该订单的所有的订单项---mapList封装的是多个订单项
				List<Map<String, Object>> mapList = service.findAllOrderItemByOid(oid);
				// 将mapList转为List<OrderItem> orderItems
				for (Map<String, Object> map : mapList) {
					try {
						// 从map中取出count subotal 封装到OrderItem中
						...
						// 将OrderItem封装到order中的orderItem中
